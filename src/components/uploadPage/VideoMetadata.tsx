...
import { THETA_API_KEY, THETA_SECRET_KEY } from "@/assets/constant";
...
export default function VideoMetadata({ videoFile , setVideoFile  }: any) {
  ...
  const [thetaState, setthetaState] = useState("")
  const [coverCID, setcoverCID] = useState();
  const [videoId, setVideoId] = useState(null); // State to store the video ID
  const [progress, setProgress] = useState(0);
  const [intervalId, setIntervalId] = useState(null);
  const [thetaVidData, setthetaVidData] = useState(null)
  const { uploadToIpfs, isUploading, isUploadingError } = usePinToIpfs();
  const [isTranscodeComplete, setisTranscodeComplete] = useState(false)
...
const [testTruth, settestTruth] = useState(true)
  ...
  console.log("the  testing part of vad progress", progress)
    console.log("the  theta videos data", thetaVidData)
  ...
  */
  ...
  /*
     ...
     */
        ...
        /*
     ===============================
       UPLOAD VIDEO TO THETA
     ==============================
     */


       const  handleUploadToTheta = async () =>  {
        ...
        setthetaState("GENERATING_URL")
        const resSignURL = await axios.post('https://api.thetavideoapi.com/upload', {}, {
          headers: {
              'x-tva-sa-id': THETA_API_KEY,
              'x-tva-sa-secret': THETA_SECRET_KEY
          }
      });

        const  uploadUrl = resSignURL?.data?.body?.uploads[0]?.presigned_url
          ...
          let generatedVidId: string = resSignURL.data.body.uploads[0].id;

          setthetaState("UPLOADING_VIDEO")
   const  uploaded = await axios.put(signedURL, videoFile, {
    headers: {
  ...
  console.log("uploadeed video", uploaded)

  //  transcode video

  let data = {
    source_upload_id:generatedVidId, // or source_uri:"link to video"
...
setthetaState("TRANSCODING_VIDEO")
...
setthetaState("FETCHING_PROGRESS")
...
const response = await axios.get('https://api.thetavideoapi.com/video/' + id, {
  headers: {
      ...
      'x-tva-sa-secret': THETA_SECRET_KEY,
        ...
        console.log("theta blockchain", resSignURL);
        
      console.log("theta blockchain", uploadUrl);
      console.log("upload id", generatedVidId);
  ...
  const fetchVideoProgress = async (videoId) => {
    try {
        ...
        const { data } = response;
        if (data && data.body && data.body.videos[0]) {
            const progress = data.body.videos[0].progress;
             console.log("the video  progress", progress)
             console.log("this  is the full progress", data)
             setthetaVidData(data?.body)
            setProgress(progress);
            if (progress === 100 && intervalId) {
              setisTranscodeComplete(true)
...
useEffect(() => {
  if (videoId) {
      const id = setInterval(() => fetchVideoProgress(videoId), 9000); // Fetch progress every 5 seconds
      setIntervalId(id); // Store the interval ID in state
      return () => clearInterval(id); // Cleanup on component unmount
  }
}, [videoId]); // Trigger effect when videoId changes
...
useEffect(() => {
 if(progress === 100){
  ...
  alert("Mission done  succfully")
     ...
     */
      ...
      // POST  END-POINT
        ...
        let postEndPoint = "https://sapo-rdii.onrender.com/api/post/create"
        ...
        try {
          ...
          const result =  await  axios.post(postEndPoint, {
            author : userProfile?.id,
             content : caption,
             title : videoTitle,
             media : thetaVidData?.videos[0]?.playback_uri
            ,
             tags : videoTags,
             appId : isShortClip ? SHORT_APP_ID : APP_ID,
             category : "finance",
             thumbnail : coverCID
          })
  ...
  const postVideo = async () => {
    ...
    //await uploadertoTus(videoFile)
  // await  createAsset();

  await   handleUploadToTheta()
   // await handlePostToBack()
  ...
  const handleCreateNote = async () => {
    ...
    try{
     ...
     /* const result = await postNote.mutate({
      ...
      const ipfsCID = await uploadToIpfs(JSON.stringify(videoMetadata))
      ...
      const createVideoData = {
        ...
        contentUri:  ipfsCID?.path,
        linkModule: "0x0000000000000000000000000000000000000000", // Use zero address for now
        linkModuleInitData: fakeLinkModuleInitData, 
        mintModule: "0x0000000000000000000000000000000000000000", // Use zero address for now
        mintModuleInitData: fakeLinkModuleInitData
    ...
    //  HANDLE  UPLOAD   TO  BACK-END 

  useEffect(() => {
    if (progress === 100  && !isPostUploaded) {
      clearInterval(intervalId)
      handlePostToBack();
    }
  }, [progress]);
  ...
  return (
    <div className="  flex  xs:flex-col md:flex-row gap-3 items-center justify-center relative  max-w-[1400px] ">
...
<div className="flex-1  h-full  px-3">
        ...
        <div className="my-4 ">
          ...
          <h1 className="text-2xl">{ assets  && assets[0]?.playbackId}</h1>
  ...
  <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12"    onClick={() => removeTag(i)}/>
        ...
        <div className="flex gap-4  my-4 text-sm w-full  px-3 justify-between border-t dark:border-gray-800 py-3">
          ...
          >
            ...
            */}
           <Button className="w-3/4 bg-green-600 text-white"
           ...
           <Button  className="py-3 px-5 rounded-xl bg-yellow-300" onClick={() => handleUploadToTheta()}>{thetaState ===  "GENERATING_URL"  ?  "Generating url"  : thetaState === "UPLOADING_VIDEO" ?  "uploading video"  :
             thetaState  === "TRANSCODING_VIDEO" ?  "transcoding video" : thetaState === "FETCHING_PROGRESS"  ?  "fetching video progress" :"upload video"
            }</Button>

          
        </div>
        <h2>hello  world: {intervalId}</h2>
      </div>
